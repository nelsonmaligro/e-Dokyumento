...
function storageImp(localStorage, serBool, value, index, boolDoc){
  return new Promise ((resolve, reject)=>{
    index.import(localStorage.getItem(value.toUpperCase()),{serialize: false, index: serBool, doc:boolDoc});
    resolve(index);
  }).catch((err)=>{});
}
...
function storageExp(localStorage, serBool, value, index, boolDoc, callback){
  new Promise((resolve, reject)=>{
    localStorage.clear();
    localStorage.setItem(value.toUpperCase(),index.export({serialize: false, index: serBool, doc:boolDoc}));
    resolve();
  }).then(()=>{
      callback();
  }).catch((err)=>{})
...
exports.findTitle = function(Path, FilDoc, FilIdx, Folder, Title, callback){
    updCollIdx(Path, FilIdx, FilDoc, Folder, function(n6Index){
      let found = n6Index.find({title:Title});
      updateCollIndex(Path, n6Index);
      callback(found);
    });
}
