try:
    ...
    def main(argv):
        ...
        try:
            ...
            colbr = mongoconek["docMS"]["classes"]
            ...
            #validate training folders and copy to retrainDoc
            if not (os.path.exists (origpath + '/retrainDoc')): os.mkdir(origpath + '/retrainDoc')
            for dir in dbBranch:
                try:
                    #if (os.path.exists(origpath + '/retrainDoc/' + dir["name"])): shutil.rmtree(origpath + '/retrainDoc/' + dir["name"])
                    if (os.path.exists (origpath + '/' + dir["name"])):
                        try:
                            if (os.path.exists(origpath + '/retrainDoc/' + dir["name"])):
                                for files in os.listdir(origpath + '/'+dir["name"]):
                                    shutil.copy2(origpath + '/'+dir["name"]+'/'+files, origpath + '/retrainDoc/'+ dir["name"])
                            else: shutil.copytree(origpath + '/'+dir["name"], origpath + '/retrainDoc/'+ dir["name"])
                            shutil.rmtree(origpath + '/' + dir["name"])
                        except: pass
                    else:
                        os.mkdir(origpath + '/retrainDoc/'+ dir["name"])
                        open(origpath + '/retrainDoc/' + dir["name"] + '/file1.txt', 'w').write(dir["name"])
                        open(origpath + '/retrainDoc/' + dir["name"] + '/file2.txt', 'w').write(dir["name"])
                        open(origpath + '/retrainDoc/' + dir["name"] + '/file3.txt', 'w').write(dir["name"])
                except: pass

            path_train = origpath + '/retrainDoc'
            # For reproducibility
            np.random.seed(1237)
